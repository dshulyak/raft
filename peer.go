...
// peerState maintans state required for the replication with a single peer.
...
func (p *peerState) lastLog() raftlog.LogEntry {
...
func (p *peerState) tick(n int) {
	p.heartbeat -= n
}

// sendHeartbeat and reset heartbeat timer.
func (p *peerState) sendHeartbeat() *AppendEntries {
...
func (p *peerState) next() *AppendEntries {
	last := p.lastLog()
...
func (p *peerState) init(m *AppendEntries) {
	p.id = m.Leader
	p.term = m.Term
	p.prevLog = m.PrevLog
	p.sentLog = m.PrevLog
...
// m1 is generated by peerState on the call to `next`.
// m2 sent by the consensus state machine.
func (p *peerState) update(m1, m2 *AppendEntries) *AppendEntries {
